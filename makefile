...
BUILDXVERSION  ?= $(call get_gh_version,docker/buildx,BUILDXVERSION)
COMPOSEVERSION ?= $(call get_gh_version,docker/compose,COMPOSEVERSION)
# MACHINEVERSION ?= $(call get_gh_version,docker/machine,MACHINEVERSION)
...
BUILDFLAGS ?= \
	...
	--build-arg BUILDXVERSION=$(if $(BUILDXVERSION),$(BUILDXVERSION),$(error BUILDXVERSION is not defined)) \
	--build-arg BUILDXARCH=$(call get_buildx_arch) \
	--build-arg COMPOSEVERSION=$(if $(COMPOSEVERSION),$(COMPOSEVERSION),$(error COMPOSEVERSION is not defined)) \
	--build-arg COMPOSEARCH=$(call get_compose_arch) \
	...
	--rm \
	--tag $(IMAGETAG) \
	# --build-arg MACHINEVERSION=$(if $(MACHINEVERSION),$(MACHINEVERSION),$(error MACHINEVERSION is not defined)) \
	# --build-arg MACHINEARCH=$(call get_machine_arch) \
	#
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e GID_DOCKER=$(shell getent group docker | awk -F ":" '{ print $$3 }') \
	# -e TZ=Asia/Kolkata \
	#  /
	# -e DOCKER_HOST=localhost:2375 \
	# -e DOCKER_HOST=ssh://username@remotehost:port \
	#  /
	# -e HOSTCWD=$(CURDIR) \
	#
...
BUILDX_ARCH_MAP := \
    ...
    #
define get_buildx_arch
$(shell case "$(ARCH)" in $(BUILDX_ARCH_MAP) esac)
endef

# maps binary platform to ARCH tags
COMPOSE_ARCH_MAP := \
	'aarch64') echo -n 'linux-aarch64' ;; \
	'armhf'  ) echo -n 'linux-armv6'   ;; \
	'armv7l' ) echo -n 'linux-armv7'   ;; \
	'x86_64' ) echo -n 'linux-x86_64' ;; \
    #
define get_compose_arch
$(shell case "$(ARCH)" in $(COMPOSE_ARCH_MAP) esac)
endef

# # maps binary platform to ARCH tags
# MACHINE_ARCH_MAP := \
# 	'aarch64') echo -n 'Linux-aarch64' ;; \
# 	'armhf'  ) echo -n 'Linux-armhf'   ;; \
# 	'armv7l' ) echo -n 'Linux-armhf'   ;; \
# 	'x86_64' ) echo -n 'Linux-x86_64' ;; \
#     #
# define get_machine_arch
# $(shell case "$(ARCH)" in $(MACHINE_ARCH_MAP) esac)
# endef
# -- }}}

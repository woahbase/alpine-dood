...
DGID       := $(shell getent group docker | awk -F ":" '{ print $$3 }')# get docker gid
...
test :
	docker run --rm -it $(NAMEFLAGS) $(MOUNTFLAGS) $(RUNFLAGS) $(PORTFLAGS) $(OTHERFLAGS) $(IMAGETAG) -ec 'docker version;  docker-machine version; docker-compose version; git --version; make -v;'
